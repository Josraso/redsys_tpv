...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='general'){
        ...
        Auth::logAction('settings_general');
    ...
    if($action==='password'){
        ...
        elseif($new!==$new2){$msg='Las contrasenas no coinciden.';$msgType='err';}
            ...
            db()->prepare('UPDATE admin_users SET password_hash=? WHERE id=?')
               ->execute(array(password_hash($new,PASSWORD_BCRYPT),Auth::userId()));
            Auth::logAction('settings_password_change');
            $msg='Contrasena cambiada.';
        }
    }
}
?>
