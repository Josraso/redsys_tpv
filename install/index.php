...
if($step===3&&$_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(empty($errors)){
        try{
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS `concepts` (`id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,`public_token` VARCHAR(32) DEFAULT NULL,`name` VARCHAR(200) NOT NULL,`description` VARCHAR(500) DEFAULT '',`icon` VARCHAR(20) DEFAULT '',`amount` DECIMAL(10,2) DEFAULT NULL,`is_libre` TINYINT(1) DEFAULT 0,`ref_prefix` VARCHAR(20) DEFAULT 'PAY',`active` TINYINT(1) DEFAULT 1,`sort_order` INT UNSIGNED DEFAULT 0,`fecha_limite` DATE DEFAULT NULL,`max_pagos` INT UNSIGNED DEFAULT NULL,`min_amount` DECIMAL(10,2) DEFAULT NULL,`max_amount` DECIMAL(10,2) DEFAULT NULL,`url_ok_custom` VARCHAR(500) DEFAULT NULL,`visible_en_index` TINYINT(1) NOT NULL DEFAULT 1,`created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,`updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
            ...
            $pdo->prepare('INSERT IGNORE INTO concepts (name,description,icon,amount,ref_prefix,active,visible_en_index,sort_order,public_token) VALUES(?,?,?,?,?,1,1,0,?)')
               ->execute(array('Concepto de ejemplo','Edita o elimina desde el admin','',10.00,'PAY',bin2hex(random_bytes(8))));
